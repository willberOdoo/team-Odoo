...
class SalesBookWizard(models.TransientModel):
    _name = 'tax.sales_book_wizard'
    _description = 'Book of every sales invoice entry by period'
    date_from = fields.Date(string='Fecha desde', required = True)
    date_to = fields.Date(string='Fecha hasta', required = True)
    related_invoices = fields.One2many(string='Facturas', comodel_name='tax.purchases_book.lines_wizard', inverse_name='related_sales_book')    
    ...
    def _compute_invoices(self):            
        ...
        for invoice in invoices:
            credit_notes = self.env['account.move'].search([('move_type','=','out_refund'),('reversed_entry_id','=',invoice.id)])
            withholding_vouchers = self.env['tax.withholding_voucher_customer'].search([('related_invoice','=',invoice.id)])     
            book_lines.append((0,0,{'name':invoice.name, 'control_number':invoice.control_number, 'total_amount_with_taxes':invoice.amount_total, 'tax_amount':invoice.amount_tax, 'tax_amount_held':0.0, 'period':invoice.invoice_date}) )
            for note in credit_notes:
                book_lines.append((0,0,{'name':note.name, 'control_number':note.control_number, 'total_amount_with_taxes':note.amount_total, 'tax_amount':0.0, 'tax_amount_held':0.0, 'period':note.invoice_date}) )
            for voucher in withholding_vouchers:
    ...
    def action_generate_xlsx_report(self):
        data = {'date_from':self.date_from, 'date_to':self.date_to, 'related_invoices': self.related_invoices.ids}
        return self.env.ref('ve_tax_withholding.action_report_excel_sales_book').report_action(self, data)

class BookReportExcel(models.AbstractModel):
    _name = 'report.ve_tax_withholding.report_book_excel_sales'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        title = workbook.add_format({'bold': True, 'align': 'center', 'font_size': 16, 'bg_color': '#f2eee4', 'border': True})
        ...
        sheet.merge_range('A1:N1', 'Libro de Ventas', title)
        ...
        sheet.merge_range('J8:K8', 'CONTRIBUYENTE', header_row_style)
        sheet.merge_range('L8:M8', 'NO CONTRIBUYENTE', header_row_style)
        row = 8
        col = 0
        ...
        sheet.write(row, col+12, 'IVA 16% ', header_row_style)
        ...
        for invoice in sorted_book_lines:
            ...
            operation_number += 1
        row += 1
        sheet.write(row , col+2, 'Resumen del Período', header_row_style)
        sheet.write(row + 1, col+1, 'Total Ventas Internas No Gravadas')
        sheet.write(row + 2, col+1, 'Total Ventas Exportación')
        sheet.write(row + 3, col+1, 'Total de las Ventas afectadas en alícuota General')
        sheet.write(row + 4, col+1, 'Total de las Ventas Internas Afectadas en alícuota Reducida')
        sheet.write(row + 5, col+1, 'Total Ventas afectadas  en alícuota General + Adicional')
        sheet.write(row + 6, col+1, 'IVA Retenido (por el Comprador)')
        sheet.write(row + 7, col+1, 'Ajuste a los Debito fiscales de períodos anteriores')
        sheet.write(row + 1, col+2, total_base)
        sheet.write(row + 2, col+2, total_credit)
        sheet.write(row + 3, col+2, total_tax)
        sheet.write(row + 4, col+2, 0.0)
        sheet.write(row + 5, col+2, 0.0)
        sheet.write(row + 6, col+2, total_tax_held)
        sheet.write(row + 7, col+2, 0.0)
